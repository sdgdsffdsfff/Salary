...
public class SalarydetailAction extends CRUDAction {
	...
	public void setSalary_detailService(
			Salary_detailService salary_detailService) {
			...
			if (listsalaryitem != null) {
				for (Salary_item salaryitem : listsalaryitem) {
					if (salaryitem.getIsedit() == 1) {
						...
						dynmaicBuffer.append("{field:\'" + salaryitem.getName()
		...
		try {
			...
			if (listsalary_item != null && !listsalary_item.isEmpty()) {
				...
				for (Salary_item salary_item : listsalary_item) {
					sqlBufferout.append("sum(field" + salary_item.getId()
							+ ") as " + salary_item.getName() + ",");
					sqlBufferin.append(" case salary_item_id when "
							+ salary_item.getId()
							+ " then money else 0 end as field"
							+ salary_item.getId() + ",");
				}
				...
				sqlBufferin.append(" from salary_detail where account_id="
						+ account_id);
				sqlBufferout.append(" from ( ");
				sqlBufferout.append(sqlBufferin);
				sqlBufferout.append(" ) as salary_item_sum");

				System.out.println("getSalarydetaillist-->sqlBufferout:"
						+ sqlBufferout.toString());
				List<Map<String, Object>> listsum = salary_detailService
						.queryNaviSql(sqlBufferout.toString(), null);
	...
	public String calcSalarydetail() {
		try {
			...
			List<Salary_item_unit> listsalary_item_unit = salary_item_unitService
					.query(unit_hql, null);
			...
			for (Salary_item_unit salary_item_unit : listsalary_item_unit) {
				mapsalary_item_unit.put(
						Integer.toString(salary_item_unit.getId()),
						salary_item_unit.getSequence());
			}
	 ...
	 * @param expressionMap
	 */
	public void calcByEmployees(List<Employee> listemployee,
			Map<String, String> mapsalary_item_unit,
